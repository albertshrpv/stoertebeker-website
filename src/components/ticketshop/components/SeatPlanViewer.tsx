...
export default function SeatPlanViewer({
    ...
    const cachedMaxScaleRef = useRef<number>(3); // Initial target max scale
    const targetMaxScaleRef = useRef<number>(3);
    const actualLayerScaleRef = useRef<number>(1); // Actual resolution scale used for layers (may be clamped)
    const MAX_TEXTURE_SIZE = 4096; // Maximum texture size in pixels
    ...
    // Calculate world bounds once when seats/legends are loaded
    const calculateWorldBounds = useCallback((seats: SeatInfo[], legends: LegendInfo[]): { minX: number; minY: number; maxX: number; maxY: number } | null => {
        ...
        if (legends.length > 0) {
            minX = Math.min(minX, ...legends.map(legend => legend.bounds.minX));
            maxX = Math.max(maxX, ...legends.map(legend => legend.bounds.maxX));
            minY = Math.min(minY, ...legends.map(legend => legend.bounds.minY));
            maxY = Math.max(maxY, ...legends.map(legend => legend.bounds.maxY));
        }
    ...
    // Recolor seats when seatGroups change (handles async updates without requiring full re-parse)
    useEffect(() => {
        // Race condition prevention: Ignore seatGroups if they don't match current svgContent
        ...
        // Update colors and group ids in-place
        let changed = false;
        for (const seat of seatsRef.current) {
        ...
        if (changed) {
            dirtyFlagsRef.current.base = true;
            // ensure layers get rebuilt on next draw
        }
    }, [seatGroups, svgContent]);
    ...
    // Helper to create offscreen canvas (with fallback to regular canvas)
    ...
    // Render base layer (non-blocked seats) in world coordinates
    const renderBaseLayer = useCallback((
        ...
        // Transform to world coordinates (offset by worldBounds.minX/minY, then scale)
        ctx.save();
    ...
    // Render blocked layer (blocked seats) in world coordinates
    ...
    // Rebuild layers when dirty flags are set
    const rebuildLayersIfNeeded = useCallback(() => {
        ...
        // Check if we need higher resolution
        ...
        // Rebuild blocked layer if needed
    ...
    const drawSeats = useCallback((ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement) => {
        ...
        if (hoveredSeat) {
            const seat = seatsByIdRef.current.get(hoveredSeat);
            if (seat) {
                // Skip wheelchair and wheelchair_accompaniment seats when not in wheelchair mode
                if (!isWheelchairMode && (seat.type === 'wheelchair' || seat.type === 'wheelchair_side' || seat.type === 'wheelchair_accompaniment')) {
    ...
    }, [selectedSeats, hoveredSeat, selectionColor, scale, offset, selectionRect, isWheelchairMode, getCachedPath, rebuildLayersIfNeeded]);
    ...
    }, [isDragging, hasDragged, isSelecting, hasSelected, isZooming, getSeatAtPoint, selectedSeats, onSeatClick, onSeatDeselect, seatGroups, scale, offset, animateZoom, isWheelchairMode, dispatch]);
    ...
    }, [isDragging, isSelecting, selectionRect, dragStart, lastOffset, getSeatAtPoint, isWheelchairMode, scale]);
    ...
    }, [isDragging, scale, hasDragged, isZooming, offset, animateZoom, getSeatAtPoint, isWheelchairMode, selectedSeats, seatGroups, onSeatClick, onSeatDeselect, dispatch]);
    ...
    }, [selectedSeats, allBlockedSeats, hoveredSeat, scale, offset, isWheelchairMode, seatGroups, drawSeats]);
